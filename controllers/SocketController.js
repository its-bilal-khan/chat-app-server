...
module.exports = {
    ...
    AddOrUpdateSocketId: function (userId, socketId) {
        ...
        let alreadyAddSocket = Object.keys(this.sockets).find( socket => socket === userId);
    ...
    OnNewUserJoin: function({accessToken}, callback, socket) {
        try {
            ...
            socket.emit('message', getMessage({user:"admin", text:`${user.details.name}, welcome to the room`}));
            
            socket.join(user.details.id);
            this.SendMessageToOthers(socket, user, {user:'loginStatus', text:{userId: user.details.id}})
    ...
    OnNewMessage : function(message, callback, socket){
        try {
            ...
            console.log(decodedMessage , typeof decodedMessage);

            const user = Users.GetUser(decodedMessage.senderId);

            if(!user || !user.isFriend(decodedMessage.receiverId)){
                console.log("No such sending user", decodedMessage.senderId, user, Users.GetAllUsers());
                return;
            }
    ...
    SocketConnect: async function(io) {
        ...
        io.on("connection", (socket)=>{
            console.log("we havee a new connection!!!!",socket.id);
            // socket.emit("message", "Working message from server")
            socket.on("join", (data, callback)=> this.OnNewUserJoin(data, callback, socket));

            socket.on("sendMessage", (message, callback)=> this.OnNewMessage(message, callback, socket))
